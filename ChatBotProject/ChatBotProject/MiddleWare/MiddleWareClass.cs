using ChatBotProject.CommandHandling;
using ChatBotProject.ConversationHistory;
using ChatBotProject.MessageRouting;
...
namespace ChatBotProject.MiddleWare
{
	public class MiddleWareClass : IMiddleware
	{
		...
		public MiddleWareClass(IConfiguration configuration)
		{
			...
			ConnectionRequestHandler connectionRequestHandler =
				new ConnectionRequestHandler(GetChannelList(KeyPermittedAggregationChannels));
		...
		public async Task OnTurnAsync(ITurnContext turnContext, NextDelegate next, CancellationToken cancellationToken = default)
		{
			...
			{
				bool.TryParse(Configuration[KeyRejectConnectionRequestIfNoAggregationChannel], out bool rejectConnectionRequestIfNoAggregationChannel);
